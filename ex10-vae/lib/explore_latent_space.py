...
def sample_on_grid(model: VAE, latent0_min: float, latent0_max: float, latent1_min: float, latent1_max: float,
    ...
    # Mesh grid tensor is reshaped from (grid_size, grid_size, 2) -> (-1, 2), where 2 is a latent_size
    reshaped_mesh_grid = mesh_grid.view(-1, 2)
    decoded = model.decode(reshaped_mesh_grid)
    # END TODO ########################
