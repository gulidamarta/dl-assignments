...
class VAE(nn.Module):
    ...
    def __init__(self, in_channels: int = 1, in_height: int = 28, in_width: int = 28, hidden_size: int = 100,
        ...
        self.fc_log_var = nn.Linear(in_features=hidden_size, out_features=latent_size)

        # Decoder Linear Layers
        self.fc_2 = nn.Linear(latent_size, hidden_size)
        self.fc_3 = nn.Linear(hidden_size, input_size)
        # END TODO ########################
    ...
    def encode(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        # START TODO ########################
        hidden_representation = self.activation(self.fc_1(x))
        mu = self.fc_mu(hidden_representation)
        logvar = self.fc_log_var(hidden_representation)
        # END TODO ########################
        return mu, logvar
    ...
    def decode(self, z: torch.Tensor) -> torch.Tensor:
        ...
        # self.activation corresponds to ReLU activation
        hidden_representation = self.activation(self.fc_2(z))
        # self.activation_output corresponds to Sigmoid activation
        output = self.activation_output(self.fc_3(hidden_representation))
        # END TODO ########################
