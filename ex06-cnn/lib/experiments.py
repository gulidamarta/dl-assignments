...
def create_conv_model() -> Module:
    ...
    conv_model = Sequential(
        Conv2d(1, n_filters_conv1, kernel_size, stride, padding),
        ReLU(),
        Conv2d(n_filters_conv1, n_filters_conv2, kernel_size, stride, padding),
        ReLU(),
        Flatten(),
        Linear(490, 10))
...
def run_shifted_conv_experiment(max_datapoints: int = 10000):
    ...
    # x_val_shifted = ...
    x_val_shifted = np.pad(x_val, ((0, 0), (0, 0), (shift, 0), (shift, 0)),
                           mode='constant')[:, :, :-shift, :-shift]
    ...
    # train
    train_results = train(mlp_model, loss_fn, optimizer, x_train_mlp, y_train, x_val_mlp, y_val, num_epochs=num_epochs,
                          batch_size=batch_size)
