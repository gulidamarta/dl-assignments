...
def calculate_conv_output(input_shape: List[int], kernel_size: List[int],
    ...
    for i in range(len(input_shape)):
        try:
            # treating x and y dimensions separately
            x_out = (input_shape[i][0] - kernel_size[i][0] + 2 * padding[i][0]) // stride[i] + 1
            y_out = (input_shape[i][1] - kernel_size[i][1] + 2 * padding[i][1]) // stride[i] + 1
            output.append((x_out, y_out))
        except TypeError:
            # if input_shape is not a list of lists, i.e. 1D input
            output.append((input_shape[i] - kernel_size[i] + 2 * padding[i]) // stride[i] + 1)
    # END TODO ################
    return output
...
class Conv2d(Module):
    ...
    def forward(self, x: np.ndarray) -> np.ndarray:
        ...
        # functions from the warmup
        def pad_x(X: np.ndarray, pad: Tuple[int, int]) -> np.ndarray:
        ...
        def dot_product_multiple_filters_with_batches(X: np.ndarray, filters: np.ndarray,
                                                      index: Tuple[int, int]) -> np.ndarray:
            result = np.sum(np.expand_dims(filters, 1) *
                            np.array(
                                [
                                    X[:, :, index[0]:index[0] + filters.shape[-2],
                                        index[1]:index[1] + filters.shape[-1]]
                                ] * filters.shape[0]
                            ),
                            axis=(2, 3, 4)).T
        ...
        # apply the functions to the input
        x_padded = pad_x(x, self.pad)
        self.input_cache = x_padded
        # calculate the feature map over entire input, taking strides into account
        for i in range(feature_map.shape[-2]):
            i_s = i * self.stride[0]
            for j in range(feature_map.shape[-1]):
                j_s = j * self.stride[1]
                h_slice = slice(i_s, i_s + self.kernel_size[0])
                w_slice = slice(j_s, j_s + self.kernel_size[1])
                feature_map[..., i, j] = dot_product_multiple_filters_with_batches(x_padded[..., h_slice, w_slice],
                                                                                   self.W.data, (0, 0))
        # add bias
