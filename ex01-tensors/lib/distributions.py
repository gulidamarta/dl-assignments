...
def plot_clt(
    n_repetitions: int = 1024,
    sample_sizes: Tuple[int, ...] = (1, 16, 64, 1024),
    plot=True,
) -> Tuple[List, List]:
    """
    ...
    fig, axes = plt.subplots(
        len(sample_sizes), len(distributions), figsize=(8, 12), squeeze=False
    )
    ...
    for nrow, n_samples in enumerate(sample_sizes):
        for ncol, (dist_name, dist_fn) in enumerate(distributions.items()):
            ...
            _, bins, _ = axis.hist(
                means, label=f"{n_samples} samples", density=True, bins=50
            )
...
def std_normal(n_samples: int = 1) -> np.ndarray:
    ...
    samples = np.random.uniform(low=-b, high=b, size=(n_samples, n_uniform_samples))
...
def plot_normal(n_samples: int = 10000, bins: int = 40) -> None:
    ...
    ax1.hist(
        np.random.normal(mean, std, n_samples), bins, label="numpy", histtype="step"
    )
    ax1.legend()
