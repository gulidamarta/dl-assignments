...
class LSTMCell(nn.Module):
    ...
    def forward(self, x: torch.Tensor, hx: Tuple[torch.Tensor, torch.Tensor] = None) \
        ...
        # Split the gate activations into four prediction for different gates
        candidate_cell_state, forget_gate, input_gate, output_gate = torch.chunk(gate_activations, 4, dim=1)

        current_cell_state = self.update_internal_state(forget_gate, previous_cell_state, input_gate,
    ...
    def update_internal_state(self, forget_gate: torch.Tensor, previous_cell_state: torch.Tensor,
        ...
        # Calculate the new internal state
    ...
    def update_hidden_state(self, current_cell_state: torch.Tensor,
        ...
        current_cell_state = torch.tanh(current_cell_state)

        new_hidden_state = output_gate * current_cell_state
        # END TODO #############
...
class NoiseRemovalModel(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        lstm1_output, _ = self.lstm1(x_padded)

        # Cut the first self.shift outputs
        lstm1_output_shifted = lstm1_output[:, self.shift:, :]

        # Forward pass through the second LSTM layer
        lstm2_output, _ = self.lstm2(lstm1_output_shifted)
