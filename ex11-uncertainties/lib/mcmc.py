...
def metropolis_hastings(target_pdf: Callable[[float], float], proposal_pdf: Callable[[float, float], float],
    ...
    for i in range(n_samples_to_generate-1):
        ...
        # get sample
        candidate = proposal_sampler(current_sample)
        # compute acceptance ratio
        acceptance_ratio = min(1, (target_pdf(candidate) * proposal_pdf(candidate, current_sample)) /
                               (target_pdf(current_sample) * proposal_pdf(current_sample, candidate) + epsilon))
        # accept candidate with probability acceptance_ratio
        if accept_with_prob(acceptance_ratio):
            # accept candidate
