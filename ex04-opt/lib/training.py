...
def create_and_train_model(opt_name: str, opt_hyperparams: Dict, x_train: np.ndarray, y_train: np.ndarray,
    ...
    optimizer = create_optimizer(name=opt_name, params=model.parameters(), hyperparams=opt_hyperparams)
...
def compare_optimizers() -> Dict[str, Tuple[List[float], List[float], List[float], List[float]]]:
    ...
    sgd = create_and_train_model(
        opt_name="sgd",
        opt_hyperparams={"lr": sgd_learning_rate},
        x_train=x_train,
        y_train=y_train,
        x_val=x_val,
        y_val=y_val,
    )
    sgd_momentum = create_and_train_model(
        opt_name="sgd",
        opt_hyperparams={"lr": sgd_learning_rate, "momentum": momentum},
        x_train=x_train,
        y_train=y_train,
        x_val=x_val,
        y_val=y_val,
    )
    adam = create_and_train_model(
        opt_name="adam",
        opt_hyperparams={"lr": adam_learning_rate},
        x_train=x_train,
        y_train=y_train,
        x_val=x_val,
        y_val=y_val,
