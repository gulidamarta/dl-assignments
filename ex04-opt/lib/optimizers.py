...
class Adam(Optimizer):
    ...
    def step(self) -> None:
        ...
        self.t += 1
        for param in self._params:
            # see slide 74 - Lecture 4
            param.state_dict["s"] = self.betas[0] * param.state_dict["s"] + (1-self.betas[0]) * param.grad
            param.state_dict["r"] = self.betas[1] * param.state_dict["r"] + (1-self.betas[1]) * param.grad**2
            s_hat = param.state_dict["s"]/(1-self.betas[0]**self.t)
            r_hat = param.state_dict["r"]/(1-self.betas[1]**self.t)
            param.grad = -(self.lr/(self.eps + r_hat**0.5))*s_hat
